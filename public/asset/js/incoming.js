...
class TripPaymentButton extends React.Component {
    render() {
        return (
            <div>
                
                <form id="DROPPED_FORM" encType="multipart/form-data" action={"/provider/request/"+this.props.request} method="POST">
                    
                     <br/>
                     <label> Total Amount </label>
                    <input type="text" name="total_amount_given" className="form-control" placeholder="Total Amount"/>
                     <input type="hidden" name="status" value="DROPPED"  />
                    <input type="hidden" name="_method" value="PATCH"  />

                    <button type="submit" className="full-primary-btn fare-btn">Payment Confirmation</button>                    
            ...
            <div className="row no-margin">
                <div className="col-md-6">
                    ...
                    <dl className="dl-horizontal left-right">
                        ...
                        {this.props.request.status == "PAYMENT" ? 

                        <dt>Total Amount</dt>
                         : ''}
                        {this.props.request.status == "PAYMENT" ? 

                        this.props.request.filter_method != null ? <dd>{this.props.currency}{this.props.request.payment.total}</dd> :<dd>{this.props.request.payment.total}</dd>
                         : ''}
...
class TripComponent extends React.Component {
    ...
    _submit(event) {
        ...
        $.ajax({
            ...
            success: function(data) {
				...
				console.log('-------------this.props.request.id-------------------------', this.props.request.id );
            }.bind(this),
            error: function(xhr, status, err) {
                console.error(this.props.url, status, err.toString());
            }.bind(this)
    ...
    getButtons() {
        switch(this.props.request.status) {
            ...
            case "PAYMENT": 
                return (
                    <TripPaymentButton request={this.props.request.id} submit={this._submit.bind(this)} />
                );
                break;
            case "DROPPED": 
                return (
