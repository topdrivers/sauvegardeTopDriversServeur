...
class UserApiController extends Controller
{
    ...
    public function send_request(Request $request) {
        ...
        // azad
        // list of providers who have riding status with distance radius
        $ProvidersOnRiding = Provider::with('service')
        ...
        $ridingProviderArr = array();    
        if(!empty($ProvidersOnRiding)) {
            foreach($ProvidersOnRiding as $pId) {
                // check destination is in radius
                $userRequestDesination = UserRequests::select(DB::Raw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(d_latitude) ) * cos( radians(d_longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(d_latitude) ) ) ) AS distance"),'current_provider_id','status','d_latitude','d_longitude')
                ->whereRaw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(d_latitude) ) * cos( radians(d_longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(d_latitude) ) ) ) <= $distance")
                ->where('current_provider_id',$pId->id)
                ->whereIn('status',array('STARTED','PICKEDUP'))
                ->first();
                if(!empty($userRequestDesination)) {
                    $ridingProviderArr[$pId->id] = $pId->id;
                }
            }
        }

        // echo "<pre>";
        //     print_r($ridingProviderArr);
        //     exit;
        // $ridingProviderArr['172'] = '172';
       
        $ProvidersOnRidingData = Provider::whereIn('id', $ridingProviderArr)->with('service')
            ->select(DB::Raw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) AS distance"),'id')
            ->where('status', 'approved')
            ->whereRaw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")
            ->whereHas('service', function($query) use ($service_type){
        ...
        if(!empty($ProvidersOnRidingData)) {
            $Providers = array_merge($ProvidersOnRidingData,$Providers);
        }    
        $Providers =$this->array_to_object($Providers); 
       // echo "<pre>";
       //      print_r($Providers);
       //      exit;
        ...
        try{
            ...
            $orgin_lat = Setting::get('orgin_latitude');
            
            $orgin_lng = Setting::get('orgin_longitude');
            
            $orgin_name = Setting::get('orgin_name');
            ...
            //Apporach distance calculation
            $apporach_details = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$orgin_lat.",".$orgin_lng."&destinations=".$request->s_latitude.",".$request->s_longitude."&mode=driving&sensor=false&key=".Setting::get('map_key');

            $apporach_json = curl($apporach_details);

            $apporach_details = json_decode($apporach_json, TRUE);

            $apporach_meter = $apporach_details['rows'][0]['elements'][0]['distance']['value'];
            $apporach_time = $apporach_details['rows'][0]['elements'][0]['duration']['text'];
            $apporach_seconds = $apporach_details['rows'][0]['elements'][0]['duration']['value'];
		
			

            //Arrived distance calculcation
            $arrived_details = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$orgin_lat.",".$orgin_lng."&destinations=".$request->d_latitude.",".$request->d_longitude."&mode=driving&sensor=false&key=".Setting::get('map_key');

            $arrived_json = curl($arrived_details);

            $arrived_details = json_decode($arrived_json, TRUE);

            $arrived_meter = $arrived_details['rows'][0]['elements'][0]['distance']['value'];
            $arrived_time = $arrived_details['rows'][0]['elements'][0]['duration']['text'];
            $arrived_seconds = $arrived_details['rows'][0]['elements'][0]['duration']['value'];

            // $approachDistance = 6;
            $approachDistance = round($apporach_meter/1000);


            // $arrivedDistance = 3;
            $arrivedDistance = round($arrived_meter/1000);
            ...
            $UserRequest->apporach_distance = $approachDistance;
            $UserRequest->arrived_distance = $arrivedDistance;
            ...
            if(Setting::get('manual_request',0) == 0){
                foreach ($Providers as $Provider) {
                    ...
                    $Filter->user_location_distance = $Provider->distance; 
                    $Filter->save();
                }
            }
    ...
    public function estimated_fare(Request $request){
        ...
        try{
            ...
            if($service_type->calculator == 'MIN') {
                ...
                $PricebyKM = $service_type->price ;

                if($arrivedDistance <= $approachDistance) {
                    


                    if ( (1/3 * $approachDistance) >= $rideDistance) { 

                        //We test if the distance of ride is lower than the third of the distance of                             approach

                       $ApproachPriceByKM = 1.4 ; // 1.4 €/ km Approach price by kilometer
 

                        $ApproachPrice = ($approachDistance) * $ApproachPriceByKM;

                       $RidePrice = ($rideDistance) * $PricebyKM;
                      
                       $calculator = 'case First';

                      }


                    if ( (2/3 * $approachDistance) >= $rideDistance &&  $rideDistance > (1/3 * $approachDistance) ) { 

                       //We test if the distance of ride is greather than the   third of the distance of approach and lower than the 2/3

                     $ApproachPriceByKM = 1 ; // 1 €/ km Approach price by kilometer
                     ...
                     if ( (2/3 * $approachDistance) < $rideDistance) { 
                     //We test if the distance of ride is greather than the 2/3 of the distance of approach

                        $ApproachPrice = 5 ;

                        $RidePrice = ($rideDistance) * $PricebyKM; 

                        $calculator = 'case Three';
            ...
            if ($arrivedDistance >= $approachDistance) {
                // We test if the customer approaches the city center
               ...
               $ApproachPriceByKM = 1.7 ; // 1.7€/km Approach price by kilometer
              ...
              if ( ((2/3 * $approachDistance) >= $rideDistance) && ($rideDistance > 1/3 * $approachDistance)) { //We test if the distance of ride is lower than the 2/3 of the distance of approach

              $ApproachPriceByKM = 1.3 ; // 1.3€/km Approach price by kilometer
             ...
             if ( (2/3 * $approachDistance) < $rideDistance &&  $rideDistance < (2 * $approachDistance)) { //We test if the distance of ride is lower than the double of the distance of approach

             $ApproachPriceByKM = 0.5 ; // 0.5€/km Approach price by kilometer
            ...
            if ( $rideDistance  > (2 * $approachDistance) )  { //si les kilomètres de la course sont supérieurs au double des km de l'approche
           ...
           if($ApproachPrice != '' && $RidePrice !=''){
                
                $price += round($ApproachPrice + $RidePrice,2);
            ...
            */ 
            ...
            return response()->json([
                    ...
                    'apporach_distance' => $approachDistance,
                    'arrived_distance' => $arrivedDistance,   
                    'orgin_name' => $orgin_name,    
                    'calculator' => $calculator,                    
                    'apporach_cost' => $ApproachPrice,
                    'ride_cost' => $RidePrice,                    
                    'Custom_Estimate_Fare' => $RidePrice,
					'approach_meter' => $apporach_meter,
    ...
    public function fare(Request $request){
        ...
        try{ 
            ...
            $allservcie = ServiceType::all();

            foreach ($allservcie as $key => $value) {
                
                $service_type = ServiceType::findOrFail($value->id);
                ...
                $allservcie[$key]->estimated_fare = round($total,2);
                $allservcie[$key]->distance = $kilometer;
                $allservcie[$key]->time = $time;
            }
